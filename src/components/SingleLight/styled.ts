...
export const Container = styled.div<{ isSp?: boolean }>`
  ...
  width: ${(props) => (props.isSp ? 88 : 178)}px;
  height: ${(props) => (props.isSp ? 88 : 178)}px;
  display: flex;
...
export const Light = styled.div<{ lightColor: string; isSp?: boolean }>`
  width: ${(props) => (props.isSp ? 52 : 110)}px;
  height: ${(props) => (props.isSp ? 52 : 110)}px;
  border-style: solid;
  border-color: ${UiColor.black};
  border-width: ${(props) => (props.isSp ? 8 : 12)}px;
